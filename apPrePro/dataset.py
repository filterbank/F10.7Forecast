...
class KpApF107Dataset (Dataset):
    ...
    def __init__(self,filePath, timestep, predlen, datatype='KP'):
    ...
    def __getitem__(self, index):
        ...
        if  self.datatype == 'KP' and len(groupData) == self.groupdaynumb * kpstepnumb:
            ...
            inputdata = np.array(groupData[:-self.predlen * kpstepnumb]).astype('float32').reshape(-1, 1)
            ...
            inputdata = np.array(groupData[:-self.predlen * apstepnumb]).astype('float32').reshape(-1, 1)
            maxvalue = np.array(np.max(inputdata))
            minvalue = np.array(np.min(inputdata))
...
if __name__ == '__main__':
    dataset_path = './data/train1968_2014.csv'
