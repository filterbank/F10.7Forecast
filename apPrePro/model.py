...
class PredictorLSTM(torch.nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, out_size):
        ...
        #                                num_layers=num_layers, batch_first=True)
        #self.lstm_line3 = torch.nn.LSTM(input_size=input_size[0], hidden_size=hidden_size[3],
        #                                num_layers=num_layers, batch_first=True)

        self.lstm_verti0 = torch.nn.LSTM(input_size=input_size[1], hidden_size=hidden_size[0],
                                        num_layers=num_layers, batch_first=True)
        self.lstm_verti1 = torch.nn.LSTM(input_size=input_size[1], hidden_size=hidden_size[1],
                                        num_layers=num_layers, batch_first=True)
        #self.lstm_verti2 = torch.nn.LSTM(input_size=input_size[1], hidden_size=hidden_size[2],
        #                                 num_layers=num_layers, batch_first=True)
        #self.lstm_verti3 = torch.nn.LSTM(input_size=input_size[1], hidden_size=hidden_size[3],
        #                                 num_layers=num_layers, batch_first=True)
        ...
        self.linear_verti = nn.Linear(hidden_size, out_size)
    ...
    def forward(self, input):
        ...
        #out_lstm_line3, _ = self.lstm_line3(out_lstm_line2[:, -1, :].reshape(input.size(0), -1, self.input_size[0]))

        out_lstm_verti0, _ = self.lstm_verti0(input.reshape(input.size(0), -1, self.input_size[1]))
        out_lstm_verti1, _ = self.lstm_verti1(out_lstm_verti0[:, -1, :].reshape(input.size(0), -1, self.input_size[1]))
        #out_lstm_verti2, _ = self.lstm_verti2(out_lstm_verti1[:, -1, :].reshape(input.size(0), -1, self.input_size[1]))
        #out_lstm_verti3, _ = self.lstm_verti3(out_lstm_verti2[:, -1, :].reshape(input.size(0), -1, self.input_size[1]))

        output = self.linear(torch.cat((out_lstm_line1[:, -1, :].squeeze(), out_lstm_verti1[:, -1, :].squeeze(),), -1))
