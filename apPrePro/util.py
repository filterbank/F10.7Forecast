...
def save_model(model, epoch, data_type = 'AP', save_model_dir = './checkpoint/model'):
...
def load_model(model, epoch = 'latest', data_type = 'AP', load_model_dir = './checkpoint/model'):
...
def plot_curve(csvfpath, datatype, savedir, load_epoch, datanumb = 27*8*2):
    ...
    for i in range(len(realvalue_list)//datanumb):
        ...
        plt.plot(plot_x, realvalue, 'g', plot_x, predvalue,'r', hold=False)
        plt.xlabel("Day")
        plt.ylabel(datatype)
...
def plot_curve_dataset(csvfpath, datatype, savedir, daynumb_img = 2*27, imgnumb=10):
    ...
    for i in range(imgnumb):
        ...
        plt.plot(plot_x, y_value, 'g', hold=False)
        plt.xlabel("Day")
        plt.ylabel(datatype)
...
if __name__ == '__main__':
    csvfpath = '/home/huangx/disk4T/xxyu/dataset/kp_ap/kp_ap/data1932_2018f107.csv'
    data_type = 'AP'
    savedir = './plot_real_value'
    if os.path.exists(savedir) is False:
        os.makedirs(savedir)
    plot_curve_dataset(csvfpath = csvfpath, datatype= data_type, savedir=savedir, daynumb_img=2* 27, imgnumb='all')
