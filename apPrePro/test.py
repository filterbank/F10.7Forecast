...
parser.add_argument('--dataset_path', type=str, default='/home/huangx/disk4T/xxyu/dataset/kp_ap/kp_ap/test2015_2018.csv', help='path of the dataset')

#****************************************#
#copy from train.py
parser.add_argument('--data_type', type=str, default='AP', help='KP|AP|F10.7')
parser.add_argument('--time_step', type=int, default= 9, help=' it indicates days')
parser.add_argument('--pred_len', type=int, default= 1, help=' it indicates days ')

parser.add_argument('--input_size', type=int, default= (1, 8), help='the length of every timestep')
parser.add_argument('--hidden_size', type=int,  default=(27*8*2, 27*8), help='the output length of every timestep ')
parser.add_argument('--num_layers', type=int, default=1, help='the number of lstm layer(vertical)')
parser.add_argument('--out_size', type=int, default=8, help='the length of predicted sequence')
...
parser.add_argument('--pred_total_len', type=int, default= 27, help=' it indicates predicted day and must be 27')
...
test_dataset = KpApF107Dataset(opt.dataset_path, timestep = opt.time_step, predlen= opt.pred_total_len, datatype=opt.data_type)
...
if opt.test_type == 'pred_total_len':
    test_dataset = KpApF107Dataset(opt.dataset_path, timestep = opt.time_step, predlen= opt.pred_total_len, datatype=opt.data_type)
    test_loader = DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=False)
    testmes = test_evaluate_model(model, test_loader, criterion, pred_len=opt.pred_len*8, test_results_path=test_results_path)

if opt.test_type == 'pred_len':
    test_dataset = KpApF107Dataset(opt.dataset_path,timestep = opt.time_step, predlen=opt.pred_len,datatype=opt.data_type)
    test_loader = DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=False)
